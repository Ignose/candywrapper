import { beretBuskingEffects, buy, canEquip, Effect, equip, equippedItem, getPower, Item, myFamiliar, myMeat, npcPrice, numericModifier, print, toEffect, toSlot, useFamiliar, useSkill } from "kolmafia";
...
function functionalPrice(item: Item): number {
  const price = have_(item) || Item.none ? 0 : npcPrice(item);
  return price;
}

function outfitPrice(outfit: { hat: Item; pants: Item; shirt: Item }): number {
  const thing = sum(relevantSlots, (slot) => functionalPrice(outfit[slot]));
  return thing;
}
...
export function reconstructOutfit(daRaw: number): { hat: Item, pants: Item, shirt: Item } {
