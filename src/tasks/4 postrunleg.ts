...
const checkMelange = () =>
  get("valueOfAdventure") * 7 * 6 > mallPrice($item`spice melange`) &&
...
export function PostRunQuests(): Quest {
  return {
    ...
    ready: () =>
      (myDaycount() === 1 || (myDaycount() === 2 && args.zooto)) && get("kingLiberated", false),
    ...
    tasks: [
      ...
      },
      {
        name: "Clear citizen",
        completed: () =>
          !get("_citizenZone", "").includes("castle") &&
      ...
      },
      {
        name: "Sober Up",
        ready: () => args.smol,
        completed: () => myInebriety() <= inebrietyLimit() || get("_mimeArmyShotglassUsed"),
        do: (): void => {
          if (checkMelange()) {
